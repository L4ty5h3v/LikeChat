// Покупка токена через прямой swap ETH/USDC → MCT внутри Farcaster
...
// Покупка токена через Farcaster Swap (прямой swap внутри Farcaster)
export async function buyTokenViaFarcasterSwap(
  ...
  // Выполняем прямой swap через Farcaster провайдер (без открытия внешних URL)
