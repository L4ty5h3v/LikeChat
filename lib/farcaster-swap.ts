// Покупка токена через прямой swap через Farcaster провайдер (транзакция в Farcaster)
...
// Покупка токена через Farcaster Swap (прямой swap через провайдер - окно транзакции в Farcaster)
export async function buyTokenViaFarcasterSwap(
  ...
  // Используем прямой swap через провайдер (окно транзакции в Farcaster)
  // Без проверки пулов (eth_call не поддерживается), пробуем все комбинации
