...
export default function Tasks() {
  ...
  const markOpened = (linkId: string) => {
    ...
    // Если проверка через API покажет, что лайка нет, задача будет показана красным
    setTasks(prevTasks => 
      prevTasks.map(task => {
      ...
      const pollIntervalId = setInterval(async () => {
        ...
        try {
          ...
          if (result.completed && isOpened) {
            ...
            // ⚠️ КРИТИЧНО: Если задача открыта, но проверка через API показала, что лайка нет
            // Показываем ошибку (красный), так как пользователь открыл задачу, но не выполнил действие
            if (!result.isError) {
              // Проверка прошла успешно, но лайка нет - это ошибка
              console.log(`❌ [POLLING] Task ${linkId} is opened, but activity not found. Showing error.`);
              taskErrorsRef.current[linkId] = true;
              setTasks(prevTasks =>
                prevTasks.map(task =>
                  task.link_id === linkId
              ...
              // Если это ошибка API (не удалось проверить), продолжаем polling
              console.log(`⏳ [POLLING] Task ${linkId} is opened, but API error. Continuing polling...`);
            }
      ...
      const updatedTasks: TaskProgress[] = await Promise.all(
        tasks.map(async (task: TaskProgress) => {
          ...
          if (!task.cast_url) {
            ...
            // Ошибка если:
            // 1. Проверка прошла успешно (!result.isError), но активность не найдена (!result.completed) И задача открыта (isOpened)
            //    - это значит пользователь открыл задачу, но не поставил лайк
            // 2. Проверка прошла успешно, но активность не найдена И задача не открыта
            const hasError = finalCompleted ? false : (
              (!result.isError) && 
              (!result.completed) // Проверка прошла, но лайка нет - это ошибка (независимо от opened)
            );
