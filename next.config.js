...
const nextConfig = {
  ...
  generateBuildId: async () => {
    // ⚠️ КРИТИЧЕСКИ ВАЖНО: Используем timestamp для уникального build ID - это заставит Vercel пересобрать все
    // ⚠️ ВАЖНО: Каждый раз при деплое генерируется новый build ID - это заставляет Vercel пересобрать ВСЕ файлы
    // ⚠️ ВАЖНО: Меняйте версию (v5, v6, etc.) при каждом деплое для очистки кеша
    const buildId = `build-${Date.now()}-no-modal-v5-force-rebuild-${Math.random().toString(36).substr(2, 9)}`;
    console.log('🔨 [BUILD] Generating new build ID (FORCE REBUILD):', buildId);
    return buildId;
  },
